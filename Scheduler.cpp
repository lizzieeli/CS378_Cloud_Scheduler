...
void Scheduler::PeriodicCheck(Time_t now) {
    ...
    // Try to assign work to standby machines BEFORE changing their states
    for (auto machine : machines_standby) {
        if (IsMachineInList(machine, machines_transitioning))
            continue;

        bool assigned_to_this_machine = false;

        // Try to assign a global pending task to this standby machine
        for (auto it = general_pending_tasks.begin();
            it != general_pending_tasks.end() && !assigned_to_this_machine; ) {

            TaskId_t task_id = *it;
            TaskInfo_t task_info = GetTaskInfo(task_id);

            if (IsMachineCapable(task_info, machine, vms)) {
                it = general_pending_tasks.erase(it);   // remove from global queue
                pending_tasks_for_machine[machine].push_back(task_id);

                assigned_to_this_machine = true;
            } else {
                ++it;
            }
        }

        // If after checking the global queue we still have nothing for this machine
        if (pending_tasks_for_machine[machine].empty() && !assigned_to_this_machine) {
            to_sleep.push_back(machine);
        } else {
            // Transition to running only if not already running/transitioning
            if (!IsMachineInList(machine, machines_running)) {
                to_run.push_back(machine);
            }
        }
    ...
    for (auto machine : to_run) {
        cout << "[Scheduler] Standby machine " << machine << " -> running S0" << endl;
        machines_running.push_back(machine);
        machines_transitioning.push_back(machine);
        RemoveMachineFromList(machine, machines_standby);
        Machine_SetState(machine, S0);
